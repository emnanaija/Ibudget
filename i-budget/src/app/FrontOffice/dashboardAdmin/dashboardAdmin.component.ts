...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, AnimatedBackgroundComponent],
...
export class DashboardAdminComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private accountsComponentRef: any = null;
  private lastScrollPosition: number = 0;
  private scrollDirection: 'up' | 'down' = 'down';
  private isTransitioning: boolean = false;
  private viewState: 'dashboard' | 'accounts' = 'dashboard';
  ...
  onWindowScroll() {
    if (!isPlatformBrowser(this.platformId) || this.isTransitioning) return;
    ...
    // Add a tolerance threshold to prevent micro-scrolls from triggering changes
    if (Math.abs(currentScrollPosition - this.lastScrollPosition) < 10) return;

    // Check if we need to handle manual transition based on scroll direction
    if (this.scrollDirection === 'up' && this.viewState === 'accounts' && currentScrollPosition < 200) {
  ...
  setupIntersectionObserver(): void {
    if (isPlatformBrowser(this.platformId)) {
      if (!this.scrollTrigger) {
        console.error('Trigger element not found in the DOM');
        return;
      }

      console.log('Setting up IntersectionObserver');
      const triggerElement = this.scrollTrigger.nativeElement;
      ...
      this.intersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting && this.scrollDirection === 'down' && this.viewState === 'dashboard') {
  ...
  switchToAccounts(): void {
    if (this.isTransitioning || this.viewState === 'accounts') return;

    this.isTransitioning = true;
    this.viewState = 'accounts';

    // Animate dashboard out
    const dashboard = this.mainDashboard.nativeElement;
    ...
    gsap.to(dashboard, {
      ...
      onComplete: () => {
        ...
        // Add this line to stop observing during transition
        if (this.intersectionObserver) {
          this.intersectionObserver.disconnect();
        }

        // Load accounts component
        this.pageContainer.clear();
        this.accountsComponentRef = this.pageContainer.createComponent(AccountsManagementComponent);
        const accountsElement = this.accountsComponentRef.location.nativeElement;

        // Initial state for accounts component
        gsap.set(accountsElement, {
          y: 100,
          opacity: 0
        });

        // Animate accounts in
        gsap.to(accountsElement, {
          y: 0,
          opacity: 1,
          duration: 0.8,
          ease: 'power3.out',
          onComplete: () => {
            ...
            // REMOVE THIS AUTOMATIC SCROLLING LINE:
            // accountsElement.scrollIntoView({ behavior: 'smooth' });

            // Instead, add a small delay before re-enabling scroll observation
            setTimeout(() => {
              this.setupIntersectionObserver();
            }, 1000);
          }
  ...
  switchToDashboard(): void {
    if (this.isTransitioning || this.viewState === 'dashboard') return;

    this.isTransitioning = true;
    this.viewState = 'dashboard';

    if (this.accountsComponentRef) {
      const accountsElement = this.accountsComponentRef.location.nativeElement;
      ...
      gsap.to(accountsElement, {
        ...
        ease: 'power3.inOut',
        onComplete: () => {
          // Remove accounts component
          this.pageContainer.clear();
          this.accountsComponentRef = null;
          ...
          gsap.fromTo(dashboard,
            { y: -100, opacity: 0 },
            {
              ...
              onComplete: () => {
                ...
                // Re-enable scroll observation with delay
                setTimeout(() => {
                  this.setupIntersectionObserver();
                }, 1000);
              }
